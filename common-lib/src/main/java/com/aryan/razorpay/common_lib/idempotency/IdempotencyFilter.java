package com.aryan.razorpay.common_lib.idempotency;

import com.aryan.razorpay.common_lib.context.MerchantContext;
import com.aryan.razorpay.common_lib.exceptions.IdempotencyConflictException;
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (!claimed) {
            ...
            if (existing.isPresent() && !IdempotencyStore.IN_PROGRESS.equals(existing.get())) {
                // it is not in progress, but coming from the actual value stored in redis
                replay(request, response, existing.get());
            } else {
                // it is still in progress by another thread
                var ex = new IdempotencyConflictException("A request with the idempotency key is in progress");
        ...
        try {
            ...
            if (status < 400 & bodyBytes.length > 0) {
                ...
                log.debug("Idempotency filter: stored response status={} key={}", status, key);
            } else {
                // error or empty - delete placeholder so client can retry cleanly
                idempotencyStore.delete(key);
                log.debug("Idempotency filter: delete placeholder after error status={} key={}", status, key);
            }
